...
def load_image(self, index):
    ...
    if img is None:  # not cached
        ...
        if switch == 0:
            # geometric scaling
            r = self.img_size / float(max(n_h, n_w))  # resize image to img_size
            if r != 1:  # always resize down, only resize up if training with augmentation
                interp = cv2.INTER_AREA if r < 1 and not self.augment else cv2.INTER_LINEAR
                img = cv2.resize(img, (int(w0 * r), int(h0 * r)), interpolation=interp)
        elif switch == 1:
            # direct scaling
            interp = cv2.INTER_AREA if random.randint(0, 1) else cv2.INTER_LINEAR
            img = cv2.resize(img, (self.img_size, self.img_size), interpolation=interp)
        elif switch == 2:
            # random scaling one edge.
            interp = cv2.INTER_AREA if random.randint(0, 1) else cv2.INTER_LINEAR
            r = self.img_size / float(max(n_h, n_w))
            if random.randint(0, 1):
                img = cv2.resize(img, (int(w0 * r), h0), interpolation=interp)
            else:
                img = cv2.resize(img, (w0, int(h0 * r)), interpolation=interp)
