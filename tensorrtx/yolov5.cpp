...
//    ITensor* data = network->addInput(INPUT_BLOB_NAME, dt, Dims3{ 3, INPUT_H, INPUT_W });
    ...
    shuffle-> setFirstTranspose(nvinfer1::Permutation{2, 0, 1});

    /* ------ yolov5 backbone------ */
//    auto focus0 = focus(network, weightMap, *data, 3, get_width(64, gw), 3, "model.0");
    auto focus0 = focus(network, weightMap,  *shuffle->getOutput(0), 3, get_width(64, gw), 3, "model.0");
...
//    Int8EntropyCalibrator2* calibrator = new Int8EntropyCalibrator2(1, INPUT_W, INPUT_H, "/mnt/data/logo/training/images/val/", "int8calib.table", INPUT_BLOB_NAME);
    Int8EntropyCalibrator2* calibrator = new Int8EntropyCalibrator2(1, INPUT_W, INPUT_H, "/home/lintao/jobs/training/logo/tensorrtx/yolov5/calib_imgs/", "int8calib.table", INPUT_BLOB_NAME);
...
void doInference(IExecutionContext& context, cudaStream_t& stream, void **buffers, float* input, float* output, int batchSize) {
    ...
    CUDA_CHECK(cudaMemcpyAsync(buffers[0], input, batchSize * INPUT_H * INPUT_W * 3 * sizeof(float), cudaMemcpyHostToDevice, stream));
...
int main(int argc, char** argv) {
    ...
    // prepare input data ---------------------------
    static float data[BATCH_SIZE * INPUT_H * INPUT_W * 3];
    ...
    CUDA_CHECK(cudaMalloc(&buffers[inputIndex], BATCH_SIZE * INPUT_H * INPUT_W * 3 * sizeof(float)));
    ...
    for (int f = 0; f < (int)file_names.size(); f++) {
        ...
        for (int b = 0; b < fcount; b++) {
            ...
            int tmp = b * INPUT_W * INPUT_H * 3;
            for (int row = 0; row < INPUT_H; ++row)
            {
                for (int col = 0; col < INPUT_W; ++col)
                {
                    data[tmp + row * INPUT_W * 3 + col * 3 + 0] = (float)pr_img.at<cv::Vec3b>(row, col)[2] / 255.0;
                    data[tmp + row * INPUT_W * 3 + col * 3 + 1] = (float)pr_img.at<cv::Vec3b>(row, col)[1] / 255.0;
                    data[tmp + row * INPUT_W * 3 + col * 3 + 2] = (float)pr_img.at<cv::Vec3b>(row, col)[0] / 255.0;
                }
            }
        }
